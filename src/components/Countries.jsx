    ...
    <section id="countries" ref={sectionRef} style={{ padding: '100px 24px', background: 'white' }}>
      <div className="container">
        <div style={{ textAlign: 'center', marginBottom: 60 }}>
          ...
          <h2 className="section-title reveal" style={{ transitionDelay: '0.1s' }}>Choose Your Country</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }} className="countries-grid">
          {countries.map(({ flag, name, tag, color, highlights, intake, cost }, i) => (
            ...
            >
              <div style={{ background: color, padding: '28px 28px 24px' }}>
                <div style={{ fontSize: '2.8rem', marginBottom: 12 }}>{flag}</div>
                <h3 style={{ color: 'white', fontFamily: 'Fraunces, serif', fontSize: '1.5rem', marginBottom: 6 }}>{name}</h3>
                <span style={{
              ...
              <div style={{ padding: '24px 28px' }}>
                ...
                <button
                  onClick={() => navigate(`/chat?country=${encodeURIComponent(name)}`)}
                  style={{
